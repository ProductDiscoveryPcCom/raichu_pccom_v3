...
try:
    ...
    AnthropicRateLimit = AnthropicAuth = AnthropicBadRequest = None

try:
    import openai as _openai_module
    OpenAIRateLimitError = _openai_module.RateLimitError
except (ImportError, AttributeError):
    OpenAIRateLimitError = None

try:
    import requests as _requests_module
    RequestsTimeout = _requests_module.exceptions.Timeout
    RequestsConnectionError = _requests_module.exceptions.ConnectionError
except (ImportError, AttributeError):
    RequestsTimeout = RequestsConnectionError = None


def _classify_error(error: Exception) -> Tuple[str, str]:
    """Classifies an exception into (error_type, user_message) for pipeline errors."""
    error_msg = str(error).lower()

    # Anthropic-specific errors
    if AnthropicRateLimit and isinstance(error, AnthropicRateLimit):
        return ("rate_limit", "Límite de tasa de la API alcanzado. Espera 30-60 segundos e intenta de nuevo.")
    if AnthropicAuth and isinstance(error, AnthropicAuth):
        return ("auth", "API key inválida o sin permisos. Verifica tu configuración en Settings.")
    if AnthropicBadRequest and isinstance(error, AnthropicBadRequest):
        if "token" in error_msg or "prompt is too long" in error_msg or "max tokens" in error_msg:
            return ("token_limit", "El prompt excede el límite de tokens. Reduce las instrucciones adicionales o el número de productos.")
        return ("bad_request", f"Solicitud inválida: {error}")

    # HTTP status code checks (for wrapped errors)
    ...
    # Token/prompt size in message
    if "prompt is too long" in error_msg or "max tokens" in error_msg or "token" in error_msg and "limit" in error_msg:
        return ("token_limit", "El prompt excede el límite de tokens. Reduce las instrucciones adicionales o el número de productos.")

    # OpenAI rate limit
    if OpenAIRateLimitError and isinstance(error, OpenAIRateLimitError):
        return ("rate_limit_openai", "Límite de tasa de OpenAI alcanzado. La corrección dual se omitirá.")

    # Timeout
    if isinstance(error, TimeoutError) or (RequestsTimeout and isinstance(error, RequestsTimeout)):
        return ("timeout", "La API no respondió a tiempo. Intenta de nuevo.")

    # Connection error
    if RequestsConnectionError and isinstance(error, RequestsConnectionError):
        return ("connection", "Error de conexión. Verifica tu conexión a internet.")
    if "connection" in error_msg and ("error" in error_msg or "refused" in error_msg):
        return ("connection", "Error de conexión. Verifica tu conexión a internet.")
...
def execute_generation_pipeline(config: Dict[str, Any], mode: str = 'new') -> None:
    ...
    # Hoisted lazy imports — loaded once per pipeline call, not per-stage
    import inspect

    try:
        from utils.serp_research import research_serp, format_for_prompt, _get_serpapi_key, format_for_display
        _serp_research_available = True
    except ImportError:
        _serp_research_available = False

    try:
        from utils.prompt_optimizer import optimize_prompt, check_prompt_size
        _prompt_optimizer_available = True
    except ImportError:
        _prompt_optimizer_available = False

    try:
        from config.arquetipos import get_structure
        _get_structure_available = True
    except ImportError:
    ...
    try:
        ...
        _keyword_analyzer_available = True
    except ImportError:
        _keyword_analyzer_available = False

    try:
        from prompts.new_content import _merge_product_data, ARQUETIPOS_CON_MINI_STORIES
        _new_content_extras_available = True
    except ImportError:
        _new_content_extras_available = False

    try:
        from utils.meta_generator import generate_meta, validate_meta
        _meta_generator_available = True
    except ImportError:
        _meta_generator_available = False

    try:
        from utils.html_utils import detect_ai_phrases
        _detect_ai_available = True
    except ImportError:
        _detect_ai_available = False
    ...
    # Contenedor de progreso con st.status()
    status_widget = st.status("Generando contenido...", expanded=True)
    progress_bar = status_widget.progress(0)
...
"""
        ...
        if mode == 'rewrite':
            ...
            if not st.session_state.get('rewrite_analysis'):
                status_widget.update(label="Etapa 0/3: Análisis Competitivo...", state="running")
                status_widget.write("🔍 Analizando contenido de competidores...")
                ...
                if not competitors_data and not html_to_rewrite:
                    status_widget.write("⚠️ No hay competidores ni contenido HTML para analizar.")
...
"""
                ...
                analysis_prompt = f"""Analiza el siguiente contenido de competidores para la keyword "{config.get('keyword', '')}".
...
Formato tu respuesta de manera clara y accionable."""
                ...
                if result.success:
                    ...
                    status_widget.write("✅ Análisis competitivo completado")
                else:
                    status_widget.write(f"⚠️ Análisis parcial: {result.error}")
        ...
        status_widget.update(label="Etapa 1/3: Generando Borrador Inicial...", state="running")
        status_widget.write("📝 Llamando a Claude API...")
        ...
        if mode == 'new':
            ...
            stage1_prompt = new_content.build_new_content_prompt_stage1(
                ...
                faq_questions=config.get('faq_questions', []),
                authoritative_sources=config.get('authoritative_sources', ''),
            )
        else:  # mode == 'rewrite'
        ...
        if _prompt_optimizer_available:
            try:
        ...
        if not result.success:
            ...
            status_widget.update(label=f"❌ Error en Etapa 1: {_err_msg}", state="error")
            st.error(f"❌ Error en Etapa 1: {_err_msg}")
            st.session_state.generation_in_progress = False
            return
        ...
        if not draft_html or len(draft_html) < 100:
            status_widget.update(label="❌ Borrador vacío o muy corto", state="error")
            st.error("❌ El borrador generado está vacío o es muy corto. Pulsa de nuevo 'Generar' para reintentar.")
            st.session_state.generation_in_progress = False
            return

        # Extraer y sanitizar HTML limpio
        raw_draft = extract_html_content(draft_html)
        st.session_state.draft_html = sanitize_html(raw_draft)

        word_count = count_words_in_html(st.session_state.draft_html)
        status_widget.write(f"✅ Borrador completado: {word_count} palabras")

        # Validar estructura CMS (3 articles requeridos)
        _cms_feedback = ""
        if validate_cms_articles:
            _cms_check = validate_cms_articles(st.session_state.draft_html)
            if not _cms_check['all_present']:
                _missing = ', '.join(_cms_check['missing'])
                logger.warning(f"Stage 1: faltan articles CMS: {_missing}")
                status_widget.write(f"⚠️ Borrador sin estructura CMS completa. Faltan: {_missing}")
                _cms_feedback = (
                    f"\n\nPROBLEMA ESTRUCTURAL CMS: El borrador no contiene los 3 <article> requeridos. "
                    f"Faltan: {_missing}. "
                    f"El HTML DEBE contener: <article class=\"contentGenerator__main\">, "
                    f"<article class=\"contentGenerator__faqs\">, "
                    f"<article class=\"contentGenerator__verdict\">."
                )
        ...
        if mode == 'new':
            ...
            try:
                ...
                if 'arquetipo_structure' in _s2_sig.parameters and _get_structure_available:
                ...
                if 'secondary_keywords' in _s2_sig.parameters:
                    stage2_kwargs['secondary_keywords'] = config.get('keywords', [])
            except Exception:
                pass
        ...
        if _cms_feedback and st.session_state.analysis_json:
            if isinstance(st.session_state.analysis_json, str):
                st.session_state.analysis_json += _cms_feedback
            elif isinstance(st.session_state.analysis_json, dict):
        ...
        if not result.success:
            _err_type, _err_msg = _classify_error(Exception(result.error)) if not isinstance(result.error, Exception) else _classify_error(result.error)
            status_widget.update(label=f"❌ Error en Etapa 3: {_err_msg}", state="error")
            st.error(f"❌ Error en Etapa 3: {_err_msg}")
            st.session_state.generation_in_progress = False
            return
        ...
        if not final_html or len(final_html) < 100:
            status_widget.update(label="❌ Contenido final vacío o muy corto", state="error")
            st.error("❌ El contenido final está vacío o es muy corto. Pulsa de nuevo 'Generar' para reintentar.")
            st.session_state.generation_in_progress = False
            return

        # Extraer y sanitizar HTML limpio
        raw_final = extract_html_content(final_html)
        st.session_state.final_html = sanitize_html(raw_final)
        ...
        try:
            if not _table_fixer_available:
                raise ImportError("table_fixer not available")
        ...
        quality_result = None
        try:
            if not _quality_scorer_available:
                raise ImportError("quality_scorer not available")
...
Genera el HTML corregido:"""
        ...
        try:
            if not _keyword_analyzer_available:
                raise ImportError("keyword_analyzer not available")
        ...
        elif _new_content_extras_available:
        ...
        _check_mini_stories = (_new_content_extras_available and _arq_code in ARQUETIPOS_CON_MINI_STORIES and _has_reviews)
        ...
        try:
            if not _meta_generator_available:
                raise ImportError("meta_generator not available")
        ...
        _err_type, _err_msg = _classify_error(e)
        try:
            status_widget.update(label=f"❌ {_err_msg}", state="error")
        except Exception:
            pass
        st.error(f"❌ Error durante la generación: {_err_msg}")
